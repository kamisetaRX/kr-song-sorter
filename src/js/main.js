...
function display() {
  ...
  document.querySelector('.right.sort.image').innerHTML =
    `<iframe width="320" height="180" src="${embedBase}${rightChar.img}" frameborder="0" allowfullscreen></iframe>`;
...
function result(imageNum = 3) {
  ...
  const imgRes = (char, num) => {
    const thumbUrl = `https://img.youtube.com/vi/${char.img}/mqdefault.jpg`;
  return `<div class="result image"><div class="left"><span>${num}</span></div><div class="right"><img src="${thumbUrl}"><div><span>${char.name}</span></div></div></div>`;
  }
  const res = (char, num) => {
    return `<div class="result"><div class="left">${num}</div><div class="right"><span>${char.name}</span></div></div>`;
  }
